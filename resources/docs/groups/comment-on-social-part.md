...
curl -X POST \
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"post_id":14,"content":"consectetur","parent_activity_id":"et","condition":{"from_id":16}}'
...
```bash
curl -X GET \
    -G "http://127.0.0.4/api/comments/6" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json"
```
...
```bash
curl -X DELETE \
    "http://127.0.0.4/api/comments/17" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"from_id":3,"to_id":5}'
